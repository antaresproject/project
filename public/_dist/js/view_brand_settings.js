...
(function(mod) {
  if (true) // CommonJS
    mod(__webpack_require__(15), __webpack_require__(212));
  else if (typeof define == "function" && define.amd) // AMD
...
(function(mod) {
  if (true) // CommonJS
    mod(__webpack_require__(15), __webpack_require__(210));
  else if (typeof define == "function" && define.amd) // AMD
...
(function(mod) {
  if (true) // CommonJS
    mod(__webpack_require__(15), __webpack_require__(714));
  else if (typeof define == "function" && define.amd) // AMD
...
(function(mod) {
  if (true) // CommonJS
    mod(__webpack_require__(15), __webpack_require__(716), __webpack_require__(211));
  else if (typeof define == "function" && define.amd) // AMD
...
var AntaresBrandColors = {
    ...
    setLastChosenColorsOnLoad: function setLastChosenColorsOnLoad() {
        setTimeout(function () {
            $('.minicolors-input.cp').each(function () {
                var colorToBeSet = $(this).val();
    ...
    defaultColors: function defaultColors() {
        function takeDefaultColor(type, order) {
            ...
            if (order === false) {
                ...
                var colorTiny = tinycolor(color);
                $('.standard-color--' + type + '-' + order).closest('.cp-brand__ar-big').find('.cp').val(colorTiny.toHexString());
            }
        }
    ...
    addStyles: function addStyles(indicator, elements, styles, value, id) {
    ...
    clearStyles: function clearStyles(indicator) {
    ...
    updateColorManually: function updateColorManually() {
    ...
    colorPickersBig: function colorPickersBig() {
        //default values
        var self = this;
        var $self = $(this);
        $('.cp-brand__ar-big input').each(function () {
            var $thisMiniColor = $(this);
            var defaultColor = $thisMiniColor.attr('value');
            $thisMiniColor.minicolors({
                ...
                change: function change(value, opacity) {
                    var $selfInChange = $(this);
                    var zeroColor = tinycolor(value).toString(),
                        darkenColor = tinycolor(value).darken(6).toString(),
                        normalColor = tinycolor(value).brighten(9).saturate(2).darken(3).toString(),
                        lighterColor = tinycolor(value).brighten(20).toString();
                    var mr = tinycolor.mostReadable(value, ['#000'], {
                    ...
                    if ($selfInChange.closest('.cp-brand--primary').length) {
                        ...
                        $selfInChange.closest('.cp-brand').find('.cp-brand__title').css('color', mr);
                        $selfInChange.closest('.cp-brand').find('input.cp').css('color', mr);
                        ...
                        var zeroMode = ['.grid-col--menu, .menu-mobile-settings', '.app-content:before', '.menu-aside-container', '.comiseo-daterangepicker .comiseo-daterangepicker-buttonpanel .btn--primary:hover'],
                            ...
                            normalMode = ['.ddown--brand .ddown__init', '#table-ma.is-disabled', '.btn.btn--brand', '.switch .switch-checkbox:checked + .switch-container', '.app-content__footer .btn--submit', '.menu-aside li.is-active a',
                        // '.mdl-textfield__label:after',
                        '.table-key .table-key__mobile-open.table-key__mobile-open--open', '.card.card--chart-small .card__header', '.card.card--chart-small .card__header-chart', '.card.card--primary-light', '.btn.btn--primary', '.select2-container .select2-selection--multiple .select2-selection__rendered li.select2-selection__choice', 'table.collapsed tbody tr.parent td:first-child:before', '.icheckbox_billevo:active', '.icheckbox_billevo:focus', '.icheckbox_billevo:hover', '#app-wrapper .icheckbox_billevo.hover', 'aside.main-sidebar .mobile-ddowns .mobile-ddowns__sgl ul.mobile-ddowns__menu', '.ddown.ddown-multi .ddown-multi__submenu > li.ddown-multi__return', '.icheckbox_billevo.checked', '.mdl-tabs.is-upgraded .mdl-tabs__tab.is-active:after', '.form-block .radio-btns [data-type="radio-btn"] input:checked + .btn', '#notification-counter, .main-head .item-grp-single .badge ', '.error-container .top-area', '.mdl-tabs__tab .mdl-tabs__ripple-container .mdl-ripple'],
                        ...
                        var primaryColorNormal = ['.grid-stack-item-content .card.card--pagination .tbl-c .pagination--type2 .dataTables_paginate span > .mdl-js-button.current', '.dataTables_length a.active', '.timeline li a', '.pagination .antares-pagination ul li .active, .pagination .antares-pagination ul li.active span', '#app-wrapper .tbl-c--zd .zd .zd__header i', '.card--info .col-group.admin', '.tbl-c table tr td a:not(.btn--primary)', '.mdl-tabs .mdl-tabs__tab.is-active', '.app-content.page-login .login-box .password-reset', '.mdl-badge--no-background', '.zd__header i',
                        // '.item-grp  i',
                        '.ddown.ddown--user .ddown__menu li a i:before', 'aside.main-sidebar .menu-tooltip .menu-tooltip__header i', '.select2-dropdown ul.select2-results__options li.select2-results__option[aria-selected="true"]:after', 'aside.main-sidebar .menu-tooltip .menu-tooltip__header span'];
                        ...
                        // gradient
                        var gradient = 'linear-gradient(to right, ' + darkenColor + ' 0%, ' + lighterColor + ' 100%)';
                        ...
                        var primaryBorderDarken = ['.antares-dropjs-filter .drop-enabled', '.table-key .table-key__input', '.icheck-label:hover > .icheckbox_billevo, .icheckbox_billevo.hover'];
                        ...
                        var thisColorPicker = $selfInChange.closest('.cp-brand').find('.cp-brand__sgl');
                        ...
                        $selfInChange.closest('.color-pickers').find('.cp-brand--typo.cp-brand--primary').find('.cp-brand__preview').css('background-color', zeroColor);
                        ...
                        //SECONDARY--------------------
                        $('.cp-brand--typo.cp-brand--secondary').find('input.cp').css('color', mr);
                        $selfInChange.closest('.cp-brand').find('.cp-brand__title').css('color', mr);
                        $selfInChange.closest('.cp-brand').find('input.cp').css('color', mr);
                        ...
                        var _thisColorPicker = $selfInChange.closest('.cp-brand').find('.cp-brand__sgl');
                        _thisColorPicker.eq(0).css('background-color', darkenColor);
                        _thisColorPicker.eq(1).css('background-color', normalColor);
                        _thisColorPicker.eq(2).css('background-color', lighterColor);
                        $selfInChange.closest('.color-pickers').find('.cp-brand--typo.cp-brand--secondary').find('.cp-brand__preview').css('background-color', zeroColor);
                        ...
                        $('.cp-brand--typo.cp-brand--tetriary').find('input.cp').css('color', mr);
                        $selfInChange.closest('.cp-brand').find('.cp-brand__title').css('color', mr);
                        $selfInChange.closest('.cp-brand').find('input.cp').css('color', mr);
                        ...
                        var _thisColorPicker2 = $selfInChange.closest('.cp-brand').find('.cp-brand__sgl');
                        _thisColorPicker2.eq(0).css('background-color', darkenColor);
                        _thisColorPicker2.eq(1).css('background-color', normalColor);
                        _thisColorPicker2.eq(2).css('background-color', lighterColor);
                        $selfInChange.closest('.color-pickers').find('.cp-brand--typo.cp-brand--tetriary').find('.cp-brand__preview').css('background-color', zeroColor);
    ...
    colorPickersSmall: function colorPickersSmall() {
        //Small color pickers
        var self = this;
        $('.cp-brand__p-sgl input').each(function () {
            ...
            $thisMiniColor.minicolors({
                ...
                change: function change(value, opacity) {
                    ...
                    if ($(this).closest('.cp-brand__p-sgl').hasClass('cp-brand__p-sgl--left')) {
                        ...
                        if ($(this).closest('.cp-brand--primary').length) {
                            ...
                            var primaryLeft = 'primary-left',
                                primaryLeftTargets = ['.menu-aside li a span, .menu-aside li a i, .menu-aside li a:after, .badge.badge--md', '.main-head .mdl-textfield--AR label.mdl-textfield__label', '.ddown.ddown--brand .ddown__menu .flex-block__title,.ddown.ddown--brand .ddown__menu .ddown__footer a', '.breadcrumbs > li:last-child .ddown__init a, .breadcrumbs > li > a,.breadcrumbs > li:last-child:before, .breadcrumbs .ddown .ddown__content .ddown__arrow:after,.breadcrumbs .ddown .ddown__init.ddown__init--white:after, .ddown .ddown__init.ddown__init--white i', '.btn.btn--brand', '.menu-mobile-settings + .select2 .select2-selection__rendered', '.ddown .ddown__init.ddown__init--white:after, .ddown .ddown__init.ddown__init--white i'];
                    ...
                    } else if ($(this).closest('.cp-brand__p-sgl').hasClass('cp-brand__p-sgl--right')) {
                        ...
                        if ($(this).closest('.cp-brand--secondary').length) {
                            ...
                            var secondaryRightTargets = ['aside.main-sidebar ul.main-menu > li > a:hover', 'aside.main-sidebar ul.main-menu > li > a:hover .icon', 'aside.main-sidebar ul.main-menu > li > a:hover i', 'aside.main-sidebar ul.main-menu > li.hovered .text', 'aside.main-sidebar ul.main-menu > li.hovered .icon', 'aside.main-sidebar .menu-scroll ul.main-menu > li.is-active > a > i', 'aside.main-sidebar .menu-scroll ul.main-menu > li.is-active > a > .text', 'aside.main-sidebar ul.main-menu > li.hovered > a i', 'aside.main-sidebar ul.main-menu .submenu section .data-list li a:hover span'];
                    ...
                    //contrast
                    var mr4 = tinycolor.mostReadable(value, ['#000'], {
                        includeFallbackColors: true,
                        level: 'AAA',
                        size: 'small'
                    }).toHexString();
...
__webpack_require__(1)(__webpack_require__(719))
...
var AntaresEmailSettings = {
    ...
    updateCodeMirror: function updateCodeMirror(data) {
    ...
    insertVariable: function insertVariable(handler) {
        this.updateCodeMirror('[[ ' + handler.attr('title') + ' ]]');
        return false;
    },
    insertInstruction: function insertInstruction(handler) {
        this.updateCodeMirror(handler.attr('title'));
        return false;
    },
    jqueryDOM: function jqueryDOM() {
        var self = this;
