...
__webpack_require__(1)(__webpack_require__(150))
...
var zeroDataAdjust = {
  ...
  adjust: function adjust() {
    ...
    $('.grid-stack-item').each(function (index, element) {
      var gsi = $(element);
      var currentWidth = gsi.attr('data-gs-width');
      var $zdElement = $(element).find('.zd');

      // datatables

      if (gsi.find('.dataTable').length && gsi.find('.dataTables_empty').length) {
        gsi.find('.card__content').css('height', 'calc(100% - 52px)');
        gsi.find('.widget-content').css('height', '100%');
        gsi.find('.card-content').css('height', '100%');
        gsi.find('table').css('height', 'calc(100% - 16px)');
        gsi.find('.dataTables_empty').css('width', '100%');
      }
...
window.zeroDataAdjust = zeroDataAdjust;

$(function () {
  zeroDataAdjust.gridstack();
  zeroDataAdjust.adjust();
});
...
var naturalSort = __webpack_require__(138),
  getByClass = __webpack_require__(72),
  extend = __webpack_require__(73),
  ...
  getAttribute = __webpack_require__(139),
  toArray = __webpack_require__(75);
...
module.exports = function(id, options, values) {
  ...
  var self = this,
    ...
    addAsync = __webpack_require__(140)(self),
    initPagination = __webpack_require__(141)(self);
  ...
  init = {
    start: function() {
      ...
      self.parse        = __webpack_require__(142)(self);
      self.templater    = __webpack_require__(143)(self);
      self.search       = __webpack_require__(144)(self);
      self.filter       = __webpack_require__(145)(self);
      self.sort         = __webpack_require__(146)(self);
      self.fuzzySearch  = __webpack_require__(147)(self, options.fuzzySearch);
...
exports.default = {
    ...
    saveGridParams: function saveGridParams() {
        ...
        if (savedPositions.length === 0) {
            ...
            $('.grid-stack-item').each(function () {
                var $this = $(this);
                var indexOfWidget = $this.attr('data-antares-grid-index');
    ...
    gsMaximize: function gsMaximize() {
        ...
        $('#app-wrapper .card .card-maximize').each(function () {
            var $maximizeButton = $(this);
            $maximizeButton.on('click', function () {
                var $self = $(this);
                var widget = $(this).closest('.grid-stack-item');
                var indexOfWidget = parseInt(widget.attr('data-antares-grid-index'), 10);
                ...
                if (!openCloseSwitch) {
                    ...
                    $('.grid-stack-item').each(function (index, el) {
                        ...
                        } else if ($(el).not('.recently-click-grid') && $(el).hasClass('is-maximized')) {} else if ($(el).not('.is-maximized') && $(el).not('.recently-click-grid')) {
                            grid.update(el, parseInt(savedPositions[index].x, 10), parseInt(savedPositions[index].y, 10), parseInt(savedPositions[index].w, 10), parseInt(savedPositions[index].h, 10));
                        }
                    });
