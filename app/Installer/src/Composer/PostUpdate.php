...
class PostUpdate extends System {
    ...
    protected function prepareDist() {
        if(is_dir($this->targetDist)) {
            ...
            $command = $this->environment->isWinOS()
                ? sprintf('rmdir %s /s /q', $this->targetDist)
                : sprintf('rm -rf %s /*', $this->targetDist);
    ...
    protected function prepareAssets() {
        ...
        if(is_dir($this->targetAssets)) {
            $command = $this->environment->isWinOS()
                ? sprintf('rmdir %s /s /q', $this->targetAssets)
                : sprintf('rm -rf %s /*', $this->targetAssets);
    ...
    protected function moveDist() {
        $command = $this->environment->isWinOS()
            ? sprintf('move %s %s', $this->sourceDist, $this->targetDist)
            : sprintf('mv %s/* %s', $this->sourceDist, $this->targetDist);
    ...
    protected function moveAssets() {
        $command = $this->environment->isWinOS()
            ? sprintf('move %s %s', $this->sourceAssets, $this->targetAssets)
            : sprintf('mv %s/* %s/', $this->sourceAssets, $this->targetAssets);
